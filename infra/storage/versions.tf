# Version constraints for this module, matching the other layers.
#
# Registry module versions are NOT captured by .terraform.lock.hcl — that file
# only locks providers. The `version` argument on the s3-bucket module block in
# main.tf is the only thing preventing a later `terraform init` from resolving a
