...
func InitFileRoutes(r *gin.Engine, config config.AWS3ConfigFile, db *gorm.DB) {
	// Initialize routes here

	route := r.Group("v1/api")
	{
		route.GET("/files", func(c *gin.Context) {
			filehandlers.DownloadFile(config, db, c)
		})
		route.POST("/files", func(c *gin.Context) {
			filehandlers.UploadFile(config, db, c)
		})
		route.DELETE("/files/:name", func(c *gin.Context) {
			filehandlers.DeleteFile(config, db, c)
		})
	}
}
